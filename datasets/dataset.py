...
def transform_dataset(name, method='ordinal', negations=False, labels='ordinal', dummy_features=None):
    """
    ...
    if dummy_features is not None:
        for i in range(dummy_features):
            table_X[f'dummy{i}'] = np.random.randint(1,4,len(table_X))
