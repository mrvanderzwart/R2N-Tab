...
def predefined_dataset(name, binary_y=False):
    ...
    ### Others
        ...
        table = pd.read_csv(dir_path + '/diabetes/diabetes.csv', header=0, skipinitialspace=True)#.dropna()
        table_X = table.iloc[:, :-1].copy()
        table_Y = table.iloc[:, -1].copy()
        categorical_cols = None
        numerical_cols = None
        ...
        table = pd.read_csv(dir_path + '/tictactoe/tic-tac-toe.csv', header=0, na_values='?', skipinitialspace=True).dropna()
        table_X = table.iloc[:, :-1].copy()
        table_Y = table.iloc[:, -1].copy()
        categorical_cols = None
        numerical_cols = None
